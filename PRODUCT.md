...
- App has a "Pair" button. Pairing mints a fresh non-exportable P-256 key. The client commits to SHA-256(pairing public key || identity public key) and the endpoint name. The server FIFO-matches two commits, then both clients reveal pairing and identity public keys. Each client checks the reveal against the commit. Pairing lasts 15 seconds, enforced by the client (ignore a late success). User can cancel. While pairing, the device shows its own LifeHash (detailed, 64×64) and a 6×4 grid of 24 BIP-39 words (full 2048-word list of SHA-256 of the pairing public key). After pair, a channel stores that pairing keypair plus the peer keys. Pairing the same two machines again updates that channel.
